...
6 which item was purchased by the customer fter they became a member ?

select * from
(select c.*,rank() over(partition by userid order by created_date) rnk from
(select a.userid,a.created_date,product_id,gold_signup_date from sales a inner join 
...
select userid,count(created_date) orders_purchased,sum(price) totwl_amt_spent from
...
9 if buying each product generates points for 5rs=2zomato points and each product has different purchasing points, (for eg. for p1 5rs=1 zomato point, for p2 10rs=5 zomato points and p3 5rs=1 zomato point) calculate points collected by each customer and for which product most points have been given till now
...
10 in the first one year after a customer joins the gold program (including their joining date) irrespective
of what the customer has purchased they earn 5 zomato points for every 10 rs spent. who earned more 1 or 3 and what was their points e
